...
exports.transport = function (conf, cb) {
	for(var moduleId in availableFiles){
		...
		if(conf.transportTpl
			&& (moduleFullPath.lastIndexOf('.html') === moduleFullPathLength - 5
			|| moduleFullPath.lastIndexOf('.tpl') === moduleFullPathLength - 4)){
		...
		// js文件
		else if(moduleFullPath.lastIndexOf('.js') === moduleFullPathLength - 3){
...
function dealFileDependency (moduleId, conf) {
	...
	// 删除注释和多余的空白等
	// var uglifiedContent = uglifyJS.minify(fileContent, {
	// 	fromString: true,
	// 	mangle: false,
	// 	compress: false
	// }).code;

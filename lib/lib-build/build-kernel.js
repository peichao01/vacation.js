...
* @param {Object} log what info to log
*/
exports.dealAllFiles = function(args){
	...
	walker.on('file', function(root, fileStats, next){
		...
		if(isURIAvailable(URI)){
			...
			log.ignore && console.log('[IGNORE] ' + URI);
		}
	});
...
function dealModuleDependency(moduleId,moduleInfo){
	...
	if(buildUtil.is(moduleInfo.uri, 'js')){
		uglifiedContent = buildUtil.getUglifiedContent(moduleContent, {
			fromString: true,
			mangle: false,
			compress: false
		}, moduleInfo.id);
...
exports.transportTemplate = function(options){
	...
	if(options.precompile){
		...
		if(options.optimize) {
			...
			var mangle = conf.uglify && conf.uglify.mangle;
			compiledTplFn = buildUtil.getUglifiedContent(prefixBeforeUglify + compiledTplFn, {
				fromString: true,
				mangle: mangle || true,
				compress: true
...
* @param {String} [idWriteOnModule = moduleId]
* @param {Array} [depsArrWriteOnModule = module.deps]
...
function getTransportedContent(moduleId, optionalArgs){
	var log = optionalArgs.log || {};

	if(moduleId.indexOf('http://') == 0){
		log.remote_module && console.log('[REMOTE MODULE] [GET CONTENT] ' + moduleId);
		return;
	}
	...
	var depsArrWriteOnModule = optionalArgs.depsArrWriteOnModule || module.deps;
	...
	// js文件
		...
		if(module.cmd === 0){
			...
			destContent = content.replace(reg, function(match, $1, $2){
				var deps = '', depsArr = depsArrWriteOnModule;//module.deps;
				// 删除对CSS的依赖，因为写在依赖中就会发起额外的请求
				if(delCssInDep){
			...
			log.not_transport && console.log('[NOT TRANSPORT]: '+moduleId);
			destContent = content;
		}
		...
		if(optionalArgs.isOptimize){
			var mangle = conf.uglify && conf.uglify.mangle;
			destContent = buildUtil.getUglifiedContent(destContent, {
				fromString: true,
				mangle: mangle || true,
				compress: true
			}, moduleId);
...
exports.getRelativeUri = function(moduleUri, relativeTo){
	var conf = getBuildConfig();
	relativeTo = relativeTo || conf.dist;
...
exports.transport = function (args) {
	var conf = getBuildConfig();
	var destDir = pth.resolve(conf.dist, args.transportDir);
	...
	for(var moduleId in allResources){
			...
			&& moduleId.trim().indexOf('http://') !== 0)
		{
			...
			// 把文件URI，从 src 搬到 dist 目录
			var destUri = exports.getRelativeUri(module.uri, destDir);
			...
			// 添加banner
			destContent = getBannerContent() + destContent;
 ...
 * @param {Object} log what info to log
 */
function getAllDeps(moduleId, allDeps, args){
	var log = (args && args.log) || {};
	// 对于远程模块不予理会
	if(moduleId.indexOf('http://') == 0) {
		log.remote_module && console.log('[REMOTE MODULE] [DEPENDENCY FIRST DEAL] ' + moduleId);
		return;
	}
...
exports.concatByPackage = function(args){
	...
	pkg.forEach(function(pkgTarget, k){
		var pkgModule = pkgTarget.module;

		if(pkgModule.id.trim().indexOf('http://') == 0){
			log.not_concat && console.log('[NOT CONCAT] package module['+pkgModule.id+'] is a remote module. ignored.');
			return;
		}

		// 只处理 JS 文件
		if(!buildUtil.is(pkgModule.id, 'js')) {
			log.not_concat && console.log('[NOT CONCAT] pkg('+pkgModule.id+') is not a js file, so will not concat this.');
			return;
		}
		var allDeps = [], mainPkg, subPkgs = [];

		getAllDeps(pkgModule.id, allDeps);

		//先过滤掉远程模块
		allDeps = allDeps.filter(function(depId){
			if(depId.trim().indexOf('http://') == 0){
				log.remote_module && console.log('[REMOTE MODULE] [DEPENDENCY IN CONCAT] ' + depId);
			}
			else return true;
		});
		...
		allDeps.forEach(function(depId){
			...
			pkgTarget.sub && pkgTarget.sub.forEach(function(sub, n){
				var subPkg = subPkgs[n] || (subPkgs[n] = []);
				sub.contain.forEach(function(containReg, i){
					if(module.uri.match(containReg)){
						// 检查是否已经被其他子包所包含，避免正则写的不严谨，一个模块被多次包含
						subPkgs.forEach(function(subPkg, j){
							if(subPkg.indexOf(depId) >= 0)
								vacation.log.error('module('+depId+') has been contained two times: config.build.pkg['+k+'].sub['+j+'] and $sub['+i+']');
						});
						// 检查是否被排除了
						pkgTarget.except.forEach(function(ex){
							if(module.uri.match(ex))
								vacation.log.error('module('+depId+') has been exceptied('+ex+') and contained(config.build.pkg['+k+'].sub['+j+']) at the same time.');
						});
						subPkg.push(depId);
					}
				});
		...
		// 主包main package 就是排除了所有 except 和 子包模块的内容
		mainPkg = allDeps.filter(function(depId, i){
			var module = resourceManager.getResource(depId);
			var r = true;
			buildUtil.each(pkgTarget.concatedExcept, function(ex){
				if(module.uri.match(ex)) return r = false;
			});
			return r;
		});
		// 因为 allDeps 肯定不会把自身算为依赖，
		// 现在是 mainPackage，自然也要把入口模块作为主包的最后一个模块
		mainPkg.push(pkgModule.id);

		subPkgs.unshift(mainPkg);
		var allPkgs = subPkgs;

		// 处理主包和子包各自应该包含哪些模块，以及 uri 和虚拟 ID（一个被加载了但是没有与之对应的模块的文件就是一个虚拟模块 -- 即包）
		var pkgNames = [];
		allPkgs.forEach(function(pkg, i){
			var isMainPkg = i === 0;
			...
			var destFileRule, destfilename, destPathInSrc, destId, destpath;
			if(isMainPkg){
				...
				destFileRule = pkgTarget.dist_rule;
				destfilename = destFileRule.replace('$file',filenameNoExt).replace('$pkg', pkgName);
			}
			else{
				// -1 是因为后来主包 unshift 进来了
				destFileRule = pkgTarget.sub[i - 1].dist_rule;
				var pkgNamesNoExt = pkg.map(function(moduleId){
					var module = resourceManager.getResource(moduleId);
					var filename = pth.basename(module.uri);
					return filename.substr(0, filename.length - pth.extname(module.uri).length);
				});
				destfilename = destFileRule.replace('$pkg', pkgName).replace('$all', pkgNamesNoExt.join('_'));
			}
			// 把 pkg 的名字替换为 dist，以便得出ID
			destPathInSrc = pth.resolve(pth.dirname(pkgModule.uri), destfilename);
			destId = getModuleInfoFromURI(destPathInSrc).id;
			destpath = exports.getRelativeUri(destPathInSrc);

			pkg.forEach(function(depId){
				var module = resourceManager.getResource(depId);
				...
				if(buildUtil.is(depId, 'css')){
					log.not_concat && console.log('[NOT CONCAT] ' + module.id);
				}
				else{
					module.pkg = pkgName;
					pkgHas.push(depId);
					log.concat && console.log('[CONCAT] ' + module.id);
				}
			});
			...
			resourceManager.setPkg(pkgName, {
				...
				name: pkgName,
				has: pkgHas,
				type: isMainPkg ? 'main' : 'sub'
			});
			pkgNames[i] = pkgName;
		});

		var needSeajsStyle = false;
		var allPkgContents= [];
		allPkgs.forEach(function(pkg, i){
			var isMainPkg = i === 0;
			...
			var fileContent = '';
			pkg.forEach(function(depId){
				// TODO:入口模块的内容在下面单独处理，之后考虑纳入循环中，历史遗留问题
				if(depId == pkgModule.id) return;

				var originDepId;
				var module = resourceManager.getResource(depId);

				if(buildUtil.is(depId, 'css')){
					var depFullPath = module.uri;
					if(args.isCssInline){
						needSeajsStyle = true;
					...
					var jsContent = ';'+getTransportedContent(depId, {
						...
						depsArrWriteOnModule: getDepsArrWriteOnModule(depId),
			...
			if(isMainPkg){
				...
				var mainFileContent = getTransportedContent(pkgModule.id, {
					delCssInDep:true,
					idWriteOnModule: resourceManager.getPkg(pkgModule.pkg).id,
					depsArrWriteOnModule: getDepsArrWriteOnModule(pkgModule.id),
					convertDependency_tpl_2_js: true,
					isOptimize: args.isOptimize,
					HandlebarsMode: args.HandlebarsMode,
					type:'concat:package',
				...
				log.concat && console.log('[CONCAT] ' + pkgModule.id);
			}
			...
			// 添加 banner
			fileContent = getBannerContent() + fileContent;

			allPkgContents[i] = fileContent;//push([destpath, fileContent]);
		});

		allPkgContents.forEach(function(pkgFileContent, i){
			var _package = resourceManager.getPkg(pkgNames[i]);
			// 必须的东西必须加到主模块才能得到保证
			// 子包的顺序不一定能得到保证
			if(needSeajsStyle && i == 0){
				var seajsTextContent = buildUtil.getUglifiedContent(pth.resolve(__dirname,'../seajs-style.js'));
				pkgFileContent = ';'+seajsTextContent+';\n' + pkgFileContent;
			}
			buildUtil.writeFile(_package.uri, pkgFileContent);
		});
		if(args.isWriteMap) exports.writeMapFile();
	});
}

// 模块有对真实模块的依赖，但在被打包后，需要把依赖写为这个包的虚拟ID，然后去重
function getDepsArrWriteOnModule(moduleId){
	var module = resourceManager.getResource(moduleId);
	var pkgs = resourceManager.getPkg();
	var depsArrWriteOnModule = [];
	module.deps.forEach(function(depId){
		var depModule, pkgName;
		// depModule 不存在，可能是依赖了远程模块
		// pkgName 找不到, 可能是模块被 expect 了，所以会不在任何一个包里
		if(!(depModule = resourceManager.getResource(depId)) || !(pkgName = depModule.pkg)){
			depsArrWriteOnModule.push(depId);
		}else{
			var pkg = pkgs[pkgName];
			if(pkg.type == 'main'){
				depsArrWriteOnModule.push(depId);
			}
			// pkg.id 都是虚拟ID，可以根据这个ID找到文件，但文件内容是其他模块们，
			else if(depsArrWriteOnModule.indexOf(pkg.id) < 0){
				depsArrWriteOnModule.push(pkg.id);
			}
		}
	});
	return depsArrWriteOnModule;
}
...
function getBannerContent(){
	...
	if(conf.uglify && conf.uglify.banner){
		banner = buildUtil.dealBanner(conf.uglify.banner);
	}
	return banner;
}
...
function _tpl_transport(res, args){
	...
	try{
		...
		log.transport && console.log('[TRANSPORT] template('+res.id+') transported.');
	}
	catch(e){

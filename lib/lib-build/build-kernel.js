...
function dealFileDependency (moduleId, conf) {
	...
	if(requireMatched){
		requireMatched.forEach(function (match) {
			...
			//console.log(file);
			file = get_real_path_by_alias(file, conf.real_alias);
			// 没有后缀名的都是 js 文件
...
// 根据 alias 得到被替换过的路径
function get_real_path_by_alias(path, aliasObj){
	var aliasPath = path;
	// 必须是 "/" 来分割目录，且不能是根目录
	...
	if(conf.paths && Object.keys(conf.paths).length > 0){
		...
		if(conf.alias && Object.keys(conf.alias).length > 0){
			for(var key in conf.alias){
				...
				real_alias[key] = get_real_path_by_alias(conf.alias[key], conf.paths);
			}
		}
	}
	else if(conf.alias && Object.keys(conf.alias).length > 0){
		for(var key in conf.alias){
			real_alias[key] = conf.alias[key];
		}
	}
	//return real_alias;

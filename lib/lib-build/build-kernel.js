...
exports.find_all_and_main_files = function (conf, cb) {
	...
	walker.on('file', function (root, fileStats, next) {
		...
		var main = Object.keys(conf.main);
...
function _transport(moduleId, conf){
	...
	// if(moduleId === 'script/aaa.js'){
	// 	console.log('===============================');
	// 	console.log(content);process.exit(0);
	// 	console.log('===============================');
	// }
	var destContent;
	// 模板文件 && 允许转换模板
	...
	// js文件
		...
		// console.log(destContent);process.exit(0);
...
exports.concatToMain = function(conf){
	//console.log(mainFiles);process.exit(0);
	// 此处假定 main 肯定是 JS 文件
	var ext = '.js',
		extlen = ext.length;
	mainFiles.forEach(function(main){
		...
		allDeps.forEach(function(depId){
			if(is(depId, 'css')){
				...
				//var filePath = pth.resolve(conf.base, depId);
				//console.log(filePath);
				fileContent += ';'+_transport(depId, conf)+'\n'; //readFile(filePath)+';\n';
			}
		});

		// console.log(conf.main);
		// console.log(filename);
		// console.log(conf.main[filename]);
		// process.exit(0);
		var destFileRule = conf.main[filename];
		var destfilename = destFileRule.replace('$1',basename);

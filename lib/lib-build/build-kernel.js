	...
	if(requireMatched){
		requireMatched.forEach(function (match) {
			...
			if(depModuleURI.indexOf('http://') == 0){
				...
				if(!depModuleID){
					vacation.log.error('[425] module('+depModuleURI+') is not found.');
				}
...
exports.transportTemplate = function(options){
	...
	if(options.transport){
		...
		destContent = 'define("'+options.moduleId+'",[],"'+destContent+'");';
	}
...
function getTransportedContent(moduleId, optionalArgs){
	...
	if(buildUtil.is(module.uri, 'html') || buildUtil.is(module.uri, 'tpl')){
		destContent = exports.transportTemplate({
			content: content,
			moduleId: moduleId,
			transport: conf.transportTpl,
			optimize: conf.optimizeTpl
		});
	}
	// js文件
...
exports.getDestUri = function(moduleUri){
	var conf = getBuildConfig();
	var destUri = pth.resolve(conf.dest, pth.relative(conf.src, moduleUri));
	return destUri;
};
...
exports.transport = function () {
	...
	for(var moduleId in allResources){
		...
		if(module.inSrc){
			...
			var destUri = exports.getDestUri(module.uri);//pth.resolve(conf.dest, pth.relative(conf.src, module.uri));
...
exports.concatByPackage = function(conf){
	dealPkgInfo();
	var conf = getBuildConfig();
	var pkg = conf.pkg;
	if(!pkg){
	...
	pkg.forEach(function(pkgTarget){
		...
		var destpath = exports.getDestUri(destPathInSrc);

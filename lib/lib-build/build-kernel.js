...
exports.concatByPackage = function(args){
	...
	pkg.forEach(function(pkgTarget){
		...
		allDeps = allDeps.filter(function(depId, i){
			...
			if(depId.trim().indexOf('http://') == 0){
				...
				//buildUtil.each(pkgTarget.sub, function(subPkg){
				//	// 顺便将 sub 的 contain 从正则替换为模块
				//	buildUtil.each(subPkg.contain, function(contain, i){
				//		if(module.uri.match(contain)){
				//			subPkg.contain[i] = module;
				//			// 因为contain 的每一个正则只
				//			return r = false;
				//		}
				//	});
				//});
				buildUtil.each(pkgTarget.except, function(ex){
		...
		var destfilename = destFileRule.replace('$1',filenameNoExt).replace('$pkg', pkgName);

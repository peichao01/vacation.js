	...
	if(requireMatched){
		requireMatched.forEach(function (match) {
			...
			if(depModuleFile.indexOf('http://') == 0){
				...
				if(!depIsExists) {
					//console.log(Linux_Style_uriFromConfigFileDir);
					console.log($3);
					console.log($4);
					console.log(depModuleURI);
					console.log(conf.ignore);
 ...
 * @param {String} [moduleId] if transport is false, not need this value
...
exports.transport = function (args) {
	...
	if(!pth.existsSync(destDir)){
		vacation.log.error('the dir that provided by option `transport` does not exists.');
	}
	var stats = pth.statsSync(destDir);
	if(!stats.isDirectory()){
		vacation.log.error('the dir that provided by option `transport` is not a directory.');
	}
	var allResources = resourceManager.getResource();
	for(var moduleId in allResources){
		var module = resourceManager.getResource(moduleId);

	...
	pkg.forEach(function(pkgTarget, k){
		...
		allPkgs.forEach(function(pkg, i){
			...
			if(isMainPkg){
				...
				destFileRule = pkgTarget.dest_rule;
				destfilename = destFileRule.replace('$1',filenameNoExt).replace('$pkg', pkgName);
			}
			else{
				// -1 是因为后来主包 unshift 进来了
				destFileRule = pkgTarget.sub[i - 1].dest_rule;
				...
				destfilename = destFileRule.replace(/\$(\d+)/g,function(m,selected){
					return pkgNamesNoExt[selected];
				}).replace('$pkg', pkgName).replace('$all', pkgNamesNoExt.join('_'));
			}
		...
		allPkgs.forEach(function(pkg, i){
			...
			if(isMainPkg){
				...
				var mainFileContent = getTransportedContent(pkgModule.id, {
					delCssInDep:true,
					idWriteOnModule: resourceManager.getPkg(pkgModule.pkg).id,//destId,//getModuleId(destpath, pth.resolve(conf.dest)),

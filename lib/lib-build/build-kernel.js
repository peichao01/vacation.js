...
exports.concatByPackage = function(args){
	...
	pkg.forEach(function(pkgTarget, k){
		...
		allPkgs.forEach(function(pkg, i){
			...
			allPkgContents.push([destpath, fileContent]);
		});

		allPkgContents.forEach(function(pkg, i){
			var destpath = pkg[0], fileContent = pkg[1];
			// 必须的东西必须加到主模块才能得到保证
			// 子包的顺序不一定能得到保证
...
function getDepsArrWriteOnModule(moduleId){
	...
	module.deps.forEach(function(depId){
		var depModule = resourceManager.getResource(depId);
		var pkgName = depModule.pkg;
		// 可能模块被 expect 了，所以会不在任何一个包里
		if(!pkgName) {

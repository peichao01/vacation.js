...
*/
function _transport(moduleId, conf, delCssInDep, idWriteOnModule){
	idWriteOnModule = idWriteOnModule || moduleId;
...
function dealFileDependency (moduleId, conf) {
	...
	if(requireMatched){
		requireMatched.forEach(function (match) {
			...
			file = deal_id_in_alias_paths(file, conf);
			// 没有后缀名的都是 js 文件
...
// 解析路径与 seajs 的 alias 和 paths 的关系
function deal_id_in_alias_paths(moduleId, conf){
	var idArr = moduleId.split('/');
	...
	if(conf.paths){
		if(conf.alias){
			for(var key in conf.alias){
				var path = conf.alias[key];
				var pathArr = path.split('/');
				pathArr.reduce(function(prev, next){
					if(prev in conf.paths){
	...
	mainFiles.forEach(function(main){
		...
		allDeps.forEach(function(depId){
			...
			if(is(depId, 'css')){
				...
				var jsContent = ';'+_transport(depId, conf, true)+'\n'; //readFile(filePath)+';\n';
		...
		// 最终输出的打包文件要更换ID
		var mainFileContent = _transport(main, conf, true, getModuleId(destpath, pth.resolve(conf.dest)));

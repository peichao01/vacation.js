...
var BaseBag = inherit({
	...
	_distRuleReplace: function(rule, pkgConf){
		...
		var dir = pth.dirname((this.isMain ? this : this.parent).mainModule.uri);
		var relativeDir = pth.relative(pkgConf.src, dir);
		// 文件所在目录就是 src 目录
		if(relativeDir === '') relativeDir = '.';
		if(rule.indexOf('$dir')>=0) rule = rule.replace('$dir', relativeDir);
		return rule;
	},

...
var BaseBag = inherit({
	...
	genDistContent: function(){
		var additionalMods = [],distContent = [];
		...
		if(this.isMain){
			...
			if(options.Handlebars == 3 && this.hasTypeModule('tpl', true)){
				additionalMods.push(buildUtil.readFile(pth.resolve(__dirname, './tpl_hb_precompile.min.js')) + spliter);
			}
			if(options.underscore == 3 && this.hasTypeModule('tpl', true)){
				additionalMods.push(buildUtil.readFile(pth.resolve(__dirname, './tpl_underscore_precompile.min.js')) + spliter);
			}
			if(options.cssinline && this.hasTypeModule('css', true)){
				...
				additionalMods.push(styleLoader({moduleType: this.moduleType}) + spliter);
			}
		}

		// TODO:被逼睡觉了，明天继续，下一步需要：如果includes 的模块要放在 top，则include的依赖也要放在top，且要在原本的 contains 中去重
		this._getModsTransportedContent(this.contains, distContent)
		this.distContent = .join('');
		//this.originContent = originContent.join('');
		return this;
	},
	_getModsTransportedContent: function(mods, container){
		var distContent = container, spliter = ';\n';
	...
	includeModules: function(modulesArray){
		var includes = [], pkgConf = buildUtil.getBuildConfig(this.pkgIndex);
		modulesArray.forEach(function(module){
			module.alldeps.concat(module).forEach(function(mod){
				if(this.contains.indexOf(mod) == -1 && includes.indexOf(mod) == -1) includes.push(mod);
			}.bind(this));
		}.bind(this));
		this.includes = includes;
		var includesContent = this._getModsTransportedContent(this.includes, []);
		if(pkgConf.includePosition == 'bottom'){
			this.distContent += includesContent.join('');
		}
		// top
		else {
			this.distContent = includesContent.join('') + this.distContent;
		}
		return this;
	},
	/**

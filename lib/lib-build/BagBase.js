...
var BaseBag = inherit({
	...
	contain: function(_mod){
		return this.contains.filter(function(mod){
			return mod == _mod;
		}).length;
	},
	...
	genDistId: function(){
		/*var distId, conf = buildUtil.getBuildConfig();
		// 把 base 在 src的相对关系，映射到 dist 目录下
		var distBase = pth.resolve(conf.dist, pth.relative(conf.src, conf.base));
		// 要发布的目录也在 dist 的 base 下，最好的一种情况
		if((distId = pth.relative(distBase, this.distFilename)).indexOf('.') != 0){
			this.distId = distId;
		}*/
		/*var mainModuleDistId = (this.isMain ? this : this.parent).mainModule.distId;
		//this.distId = pth.relative(conf.src, pth.resolve(this.distDirname, this.distFilename));
		this.distId = pth.join(pth.dirname(mainModuleDistId), this.distFilename);*/
	...
	getDistId: function(fileFullPath){
		...
		return pth.relative(pth.dirname(fileFullPath), this.distFilename);
	},
	...
	genDistContent: function(){
		...
		if(this.isMain){
			...
			if(options.cssinline && this.hasTypeModule('css', true)){
				distContent.push(buildUtil.readFile(pth.resolve(__dirname, './seajs-style.js')) + spliter);
			}
		}
	...
	hasTypeModule: function(type, inAllModules){
		...
		var mods = inAllModules ? (this.isMain ? this : this.parent).mainModule.alldeps : this.contains;

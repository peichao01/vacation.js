...
// 生成可读的路径，有的路径实在是太长了
exports.getReadableUri = function(uri){
	return uri.substr(uri.length - 70);
};
...
exports.writeFile = function(filePath, content, callback, _checked) {
	try{
		...
		if(options.autocheckout && !_checked){
			...
			vacation.log.debug('[WRITE FAILED] ' + filePath);
 ...
 * @param {Number} quoteMode 1 双引号，2 单引号，3 单双引号都处理
 * */
exports.content2StandardString = function(content, quoteMode){
	quoteMode = quoteMode || 1;
	var quoteReg = quoteMode == 1 ? /(")/g : quoteMode == 2 ? /(')/g : /('|")/g;
 ...
 */
exports.isURIExcepted = function(options){
	var pkgConf = exports.getBuildConfig(options.pkgIndex);
	var Linux_Style_Uri = exports.normalize_win_dir(options.uri);
	//console.log(Linux_Style_Uri);
	var r = !pkgConf.except || pkgConf.except.some(function(except_reg){
		return Linux_Style_Uri.match(except_reg);
	});
	return r;
};
 ...
 * @param {Number} pkgIndex
 */
exports.isIncluded = function(options){
	var pkgConf = exports.getBuildConfig(options.pkgIndex);
	var Linux_Style_Uri = exports.normalize_win_dir(options.uri);
	var r = pkgConf.include && pkgConf.include.some(function(include_reg){
		return Linux_Style_Uri.match(include_reg);
	});
	return r;
};

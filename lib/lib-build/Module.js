...
function deepDependency(_module, alldeps, options){
	...
	_module.deps.forEach(function(depModule){
		if(!options.without_excepted || !depModule.isMeExcepted){
			var depModuleAllDeps = depModule.alldeps; //options.without_excepted ? depModule.alldeps_without_excepted : depModule.alldeps;
...
var Module = inherit({
	...
	__constructor: function(opt){
		this.uri = opt.uri;
		// 有的路径太长了...
		// chrome 的开发工具，Scope Variables 中可以显示 70多个字符
		this.debugUri = buildUtil.getReadableUri(this.uri); //this.uri.substr(this.uri.length - 70);
	...
	_check: function(){
		...
		if(uriCaches[this.uri]) {
			vacation.log.error('circle dependencies.');
		}
	...
	baseInfo: function(){
		...
		var debugDistUri = this.distUri.substr(80);
		...
		// 先使用根据部署目录来生成id，没有id的话，则使用根据 src 目录生成的id
		this.id = buildUtil.normalize_win_dir(someInfo.id) || this.id_BySrc;
		var needId;
		// remote模块不需要id
		if(this.isRemote){
			needId = false;
			...
			/*
			* 需要 concat 或需要 transport 的模块，需要id
			 */
			if(options.concat || options.transport){
				needId = true;
			}
			// 事实上，vacation tpl 的功能就不需要ID，因为只是将 模版 处理为 js模块，不需要 id，这也符合 options.concat 和 options.transport 的判断
			else {
				needId = false;
			}
		}
		if(!this.id && needId){
			vacation.log.error('can not generate a great id for the module['+uri+']');
		}
		/**
		 * requireJS 的规则：
		 *
		...
		if(this.haveTextPrefix){
			this.distId = 'text!' + this.distId;
			this.distId_BySrc = 'text!' + this.distId_BySrc;
		}

		// [DEBUG]
//		if(this.debugUri.lastIndexOf('c.ui.core.js') >= 0){
//			'';
//		}

		// 如果没有，则使用根据 src 目录生成的 distId
		// id 已经做过兼容处理了，这里就没必要了
		// this.distId = this.distId || this.distId_BySrc;
		...
		if(this.type == 'js'){
			...
			if(this.cmd === 0){
				...
				requiredModules.forEach(function(depModuleFile){
					...
					var unixDepModuleURI = buildUtil.normalize_win_dir(depModuleURI);
					// 此模块需要被忽略
					if(pkgConf.ignore
						&& pkgConf.ignore.length
						&& pkgConf.ignore.some(function(reg_ignore){return unixDepModuleURI.match(reg_ignore);}))
					{
						ignoredDeps.push(depModuleURI);
					}
					else{
						var depModule = this.__self.get({
							...
							requiredBy: this,
							pkgIndex: this.pkgIndex,
							haveTextPrefix: haveTextPrefixMods.indexOf(depMod) >= 0
						});
						deps.push(depModule);
					}
				}.bind(this));
	...
	transport: function(){
		...
		if(this.isTpl){
			...
			var the_first_two_params = opt.concat ? ('"'+distId+'",[],') : '';
			if(opt.Handlebars >= 2){
		...
		// [seajs源码]： if (exports === null && !IS_CSS_RE.test(uri)) { emit("error", mod) }
			...
			if(opt.cssinline){
				...
				content = this.moduleType + '.importStyle("'+buildUtil.content2StandardString(content, 3)+'");';
			}
			else content = '';
		}
		else if(this.type == 'js'){
			// [DEBUG]
//			if(this.filename == 'c.ui.core'){
//				'';
//			}
			if(this.cmd == 0){

...
function deepDependency(_module, alldeps, options){
	...
	_module.deps.forEach(function(depModule){
		...
		depModuleAllDeps.forEach(function(mod){
			if(_deepDependency_check_mod_available(mod, alldeps, options)) alldeps.push(mod);
		});
		if(_deepDependency_check_mod_available(depModule, alldeps, options)) alldeps.push(depModule);
	});
	return alldeps;
}
//deepDependency中需要判断
function _deepDependency_check_mod_available(mod, alldeps, options){
	//1. 去重
	return alldeps.indexOf(mod) < 0 &&
		//2.无需检查【排除】；或者，检查【排除】的了，但没有被排除
		(!options.without_excepted || !mod.isMeExcepted)
		//(!options.without_excepted || !buildUtil.isURIExcepted(mod.uri))
}

...
var Module = inherit({
	...
	baseInfo: function(){
	     ...
	     */
		this.distId = (this.isRequireJsModule() || !this.isTpl)
	...
	_checkId: function(moduleId){
		...
		var conf = buildUtil.getBuildConfig();
		// 如果只能使用相对路径做标识ID，则报错
		if(this.cmd === 0 && !moduleId && conf.transport && !this.isRemote){
			console.log('\n [HELP INFO] paths and alias is parsed to: ' + JSON.stringify(conf.real_alias_rootPathed, null, 4));
			var info = this.isMain ? 'is main module' : ('required by "'+(this.requiredBy[0] ? this.requiredBy[0].uri : '')+'"');
			vacation.log.error('[423] module(uri:'+this.uri+')('+info+') not in the base directory('+conf.base+'), and no paths or alias relative to its path.');
		}
	},

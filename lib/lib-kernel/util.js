...
///
var configFileName = 'vacation.json';
// from current dir to root to find the config file
// first the cwd() directory than the $vacation_root dir
function getFileIteratively(dir, file){
	var dirArr = dir.split(_.dirSpliter),
		confFilePath,
		findConf;
		
    //console.log(dirArr,'------');
	for(var i=0,len=dirArr.length; i<len; i++){
		confFilePath = pth.resolve(dirArr.join(_.dirSpliter), file);
		if(_exists(confFilePath)){
			findConf = true;
			break;
		}
		dirArr.pop();
	}
	return findConf && confFilePath;
}

_.getConfigFilePath = function(){
	return getFileIteratively(process.cwd(), configFileName);
};

/*
 * @param {String} [confFile]
 */
_.getConfig = function(confFile){
	//dirpath = _.realpath(dirpath);
	confFile = confFile ||  _.getConfigFilePath();
				//|| getFileIteratively(dirpath, configFileName);

	if(confFile){
		return _.readJSON(confFile);
	} else {
		return {};//vacation.log.error('no config file('+ configFileName +') founded.');
	}
};
///
...
_.touch = function(path, mtime){
    ...
    if(mtime instanceof Date){
        ...
        vacation.log.error('invalid argument [mtime]');
    }
...
function getFileTypeReg(type){
    var map = [], ext = vacation.config.get('project.fileType.' + type);
    if(type === 'text'){
        map = TEXT_FILE_EXTS;
    } else if(type === 'image'){
        map = IMAGE_FILE_EXTS;
    } else {
        vacation.log.error('invalid file type [' + type + ']');
    }
    if(ext && ext.length){
...
_.md5 = function(data, len){
    ...
    len = len || vacation.config.get('project.md5Length', 7);
...
_.read = function(path, convert){
    ...
    if(_exists(path)){
        ...
        vacation.log.error('unable to read file[' + path + ']: No such file or directory.');
    }
    return content;
};
...
_.find = function(rPath, include, exclude){
    ...
    if(path){
        ...
        vacation.log.error('unable to find [' + rPath + ']: No such file or directory.');
    }
    return list.sort();
};
...
_.del = function(rPath, include, exclude){
    ...
    if(path) {
        if(/^(?:\w:)?\/$/.test(path)){
            vacation.log.error('unable to delete directory [' + rPath + '].');
        }
        var stat = fs.statSync(path);
        if(stat.isDirectory()){
        ...
        //vacation.log.error('unable to delete [' + rPath + ']: No such file or directory.');
    }
    return removedAll;
};
...
_.copy = function(rSource, target, include, exclude, uncover, move){
    ...
    if(source){
        ...
        vacation.log.error('unable to copy [' + rSource + ']: No such file or directory.');
    }
    return removedAll;
};
...
_.pipe = function(type, callback, def){
    var processors = vacation.config.get('modules.' + type, def);
    if(processors){
        ...
        processors.forEach(function(processor, index){
            ...
            if(typeOf === 'string'){
                key = type + '.' + processor;
                processor = vacation.require(type, processor);
            } else {
                key = type + '.' + index;
            }
            if(typeof processor === 'function'){
                var settings = vacation.config.get('settings.' + key, {});
                if(processor.defaultOptions){
                    settings = _.merge(processor.defaultOptions, settings);
                }
                callback(processor, settings, key);
            } else {
                vacation.log.warning('invalid processor [modules.' + key + ']');
            }
        });
    }
};
...
_.download = function(url, callback, extract, opt){
    ...
    var http = opt.protocol === 'https:' ? require('https') : require('http'),
        ...
        tmp  = vacation.project.getTempPath('downloads', name),
        data = opt.data;
    delete opt.data;
    _.write(tmp, '');
    var writer = fs.createWriteStream(tmp),
        http_err_handler = function(err){
            ...
            if(callback){
                ...
                vacation.log.error('request error [' + msg + ']');
            }
        },
                            ...
                            fs
                                ...
                                .on('error', function(err){
                                    if(callback){
                                        ...
                                        vacation.log.error('extract tar file [' + tmp + '] fail, error [' + err + ']');
                                    }
                                })
                                .on('end', function(){
...
_.upload = function(url, opt, data, content, subpath, callback){
    if(typeof content === 'string'){
        ...
        vacation.log.error('unable to upload content [' + (typeof content) + ']');
    }
    data = data || {};
    var endl = '\r\n';
    ...
    _.download(url, function(err){
        if(err){
            if(opt.error){
                ...
                vacation.log.error( 'unable to download component [' +
...
_.nohup = function(cmd, options, callback){
    ...
    if(IS_WIN){
        var cmdEscape = cmd.replace(/"/g, '""'),
            file = vacation.project.getTempPath('nohup-' + _.md5(cmd) + '.vbs'),
            script = '';
        script += 'Dim shell\n';
        ...
        return exec('cscript.exe /nologo "' + file + '"', options, function(error, stdout){
            if(stdout != '0'){
                vacation.log.error('exec command[' + cmd + '] fail.');
            }
            fs.unlinkSync(file);
        ...
        return exec('nohup ' + cmd + ' > /dev/null 2>&1 &', options, function(error, stdout){
            if(error !== null) {
                vacation.log.error('exec command[' + cmd + '] fail, stdout [' + stdout + '].');
            }
            if(typeof callback === 'function') {
                callback();
            }
